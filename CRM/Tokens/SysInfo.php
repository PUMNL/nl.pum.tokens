...
class CRM_Tokens_SysInfo {
  ...
  public function tokenValues(&$values, $cids, $job = null, $tokens = array(), $context = null) {
	$config = CRM_Core_Config::singleton();
	$this->datetime = CRM_Utils_Time::getTime();
    if ($this->checkToken($tokens, 'datetime')) {
      $this->dateTimeToken($values, $cids, 'datetime', $config->dateformatDatetime);
    }
	if ($this->checkToken($tokens, 'fulldate')) {
      $this->dateTimeToken($values, $cids, 'fulldate', $config->dateformatFull);
    }
	if ($this->checkToken($tokens, 'partialdate')) {
      $this->dateTimeToken($values, $cids, 'partialdate', $config->dateformatPartial);
    }
	if ($this->checkToken($tokens, 'year')) {
      $this->dateTimeToken($values, $cids, 'year', $config->dateformatYear);
    }
	if ($this->checkToken($tokens, 'time')) {
      $this->dateTimeToken($values, $cids, 'time', $config->dateformatTime);
    }
  }
  ...
  private function dateTimeToken(&$values, $cids, $token, $format=null) {
    $value = $this->datetime;
    foreach($cids as $cid) {
      $values[$cid][$this->token_name.'.'.$token] = CRM_Utils_Date::customFormat($value, $format); // $format . ' ' . date($format, $value);
