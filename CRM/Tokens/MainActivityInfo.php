...
class CRM_Tokens_MainActivityInfo {
  ...
  public function __construct($token_name, $token_label, $case_id = null) {
	...
	if(!empty($this->case_id)) {
      ...
      $api_result = civicrm_api3('CustomGroup', 'get', array('name' => 'main_activity_info'));
      ...
      if (!empty($customfields)) {
        try{
          ...
          if($dao->N == 1) {
            $dao->fetch();
            $this->startdate = $dao->main_activity_start_date;
            $this->enddate = $dao->main_activity_end_date;
          }
        } catch (Exception $e) {
          //do nothing
        }
